...
#define SOLUTION_SIZE ((2^WK)*(WN/(WK+1)+1))
        ...
        while (true) {
            ...
            while (true) {
				...
				(const std::vector<uint32_t>& index_vector, size_t cbitlen, const unsigned char* compressed_sol) 
				{
					...
					if (compressed_sol)
					{
						actualHeader.nSolution = std::vector<unsigned char>(SOLUTION_SIZE);
                    ...
                    std::string s = hexStr((unsigned char *)actualHeader.nSolution.data(), actualHeader.nSolution.size());
                    BOOST_LOG_CUSTOM(info, pos) << "Solution: " << s;
...
ZcashJob* ZcashMiner::parseJob(const Array& params)
{
    ...
    unsigned int major;
    sscanf(params[1].get_str().c_str(), "%x", &major);

    int32_t version;
    sscanf(params[2].get_str().c_str(), "%x", &version);
    // TODO: On a LE host shouldn't this be le32toh?
    /*
    std::cout << "-HEADER-" << std::endl;
    std::cout << "JobID: " << params[0].get_str() << std::endl;
    std::cout << "Major: " << params[1].get_str() << " (" << major << ")" << std::endl;
    std::cout << "Minor: " << params[2].get_str() << std::endl;
    std::cout << "hashPrevBlock: " << params[3].get_str() << std::endl;
    std::cout << "hashMerkleRoot: " << params[4].get_str() << std::endl;
    std::cout << "hashWitnessMerkleRoot: " << params[5].get_str() << std::endl;
    std::cout << "nTime: " << params[6].get_str() << std::endl;
    std::cout << "nBits: " << params[7].get_str() << std::endl;
    std::cout << "clean: " << params[8].get_bool() << std::endl;
    */

    ret->header.nMajorVersion = major;
    ret->header.nMinorVersion = be32toh(version);

    if (ret->header.nMinorVersion == 4 || ret->header.nMinorVersion == 0x20000000) {
        if (params.size() < 9) {
            throw std::logic_error("Invalid job params");
        }
