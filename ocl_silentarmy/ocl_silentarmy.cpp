...
bool OclContext::init(cl_device_id dev, unsigned int threadsNum, unsigned int threadsPerBlock)
{
    /*
	cl_int error;
    ...
    */
    ...
    queue = clCreateCommandQueue(_context, dev, 0, &status);
    ...
    source = ocl_code.c_str();
    source_len = ocl_code.length();// strlen(ocl_code);
    _program = clCreateProgramWithSource(_context, 1, (const char **)&source, &source_len, &status);
    if (status != CL_SUCCESS || !_program)
        fatal("clCreateProgramWithSource (%d)\n", status);
    /* Build program. */
    //if (!mining || verbose)
    //    fprintf(stderr, "Building program\n");
    status = clBuildProgram(_program, 1, &dev,
                            "", // compile options
                            NULL, NULL);
    if (status != CL_SUCCESS)
    {
        warn("OpenCL build failed (%d). Build log follows:\n", status);
        get_program_build_log(_program, dev);
        exit(1);
    }
    //get_program_bins(program);
    // Create kernel objects
    k_init_ht = clCreateKernel(_program, "kernel_init_ht", &status);
    ...
    for (unsigned round = 0; round < PARAM_K; round++)
    {
        ...
        k_rounds[round] = clCreateKernel(_program, name, &status);
        if (status != CL_SUCCESS || !k_rounds[round])
            fatal("clCreateKernel (%d)\n", status);
    }
    k_sols = clCreateKernel(_program, "kernel_sols", &status);
    if (status != CL_SUCCESS || !k_sols)
        fatal("clCreateKernel (%d)\n", status);
    
    if (!(dbg = calloc(dbg_size, 1)))
        fatal("malloc: %s\n", strerror(errno));
    buf_dbg = check_clCreateBuffer(_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, dbg_size, dbg);
    buf_ht[0] = check_clCreateBuffer(_context, CL_MEM_READ_WRITE, HT_SIZE, NULL);
    buf_ht[1] = check_clCreateBuffer(_context, CL_MEM_READ_WRITE, HT_SIZE, NULL);
...
*/
void ocl_sort_pair(uint32_t *a, uint32_t len)
{
	uint32_t    *b = a + len;
...
static uint32_t verify_sol(sols_t *sols, unsigned sol_i)
{
	...
	for (uint32_t level = 0; level < PARAM_K; level++)
		for (i = 0; i < (1 << PARAM_K); i += (2 << level))
			ocl_sort_pair(&inputs[i], 1 << level);
	return 1;
}
...
void ocl_silentarmy::start(ocl_silentarmy& device_context) {
	...
	for (unsigned i = 0; i < gpus.size(); i++) {
		...
		cl_int error;
		device_context.oclc->_context = clCreateContext(NULL, 1, &gpus[i], NULL, NULL, &error);
		//OCLR(error, false);
		if (cl_int err = error) {
...
{
	if (device_context.oclc != nullptr)
        delete device_context.oclc;
}
	...
	ocl_silentarmy& device_context)
{
    ...
    cl_mem buf_blake_st;
    buf_blake_st = check_clCreateBuffer(miner->_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(blake2b_state_s), &initialCtx);
