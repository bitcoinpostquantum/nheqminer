...
eq_cuda_context::eq_cuda_context(int tpb, int blocks, int id) : threadsperblock(tpb), totalblocks(blocks), device_id(id)
{
  ...
  int nthreads = 1024;
  threadsperblock = 0;
  if (!threadsperblock) // if not set, then default threads per block to roughly square root of threads
    for (threadsperblock = 1; threadsperblock*threadsperblock < nthreads; threadsperblock *= 2) ;

  totalblocks = nthreads / threadsperblock;    
